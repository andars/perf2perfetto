...
pub unsafe extern "C" fn filter_event_early(
    ...
    if (*sample.event) == 'b' as c_char {
        ...
        // since the last sample or it grew by more than BAD_JUMP_HEURISITC,
...
mod ftf {
    ...
    impl Default for StringCache {
        fn default() -> Self {
            ...
            let lru = LruCache::new(STRING_TABLE_SIZE);
            Self { lru }
        }
    }
    ...
    impl StringCache {
        ...
        pub fn get_ref(&mut self, w: &mut dyn Write, s: &str) -> std::io::Result<CacheRef> {
            ...
            if let Some(idx) = self.lru.get(s) {
                ...
                let idx = if self.lru.len() < self.lru.cap() {
                    self.lru.len() as u16
                } else {
    ...
    impl Default for ThreadCache {
        fn default() -> Self {
            ...
            let lru = LruCache::new(THREAD_TABLE_SIZE);
            Self { lru }
        }
    }
